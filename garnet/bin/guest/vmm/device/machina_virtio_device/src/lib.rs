...
pub struct QueueConfig {
    ...
    avail: DriverAddr,
    used: DriverAddr,
    responder: fidl_fuchsia_guest_device::VirtioDeviceConfigureQueueResponder,
}

pub enum DeviceRecv {
    Queue(QueueConfig),
    Ready(u32, fidl_fuchsia_guest_device::VirtioDeviceReadyResponder),
}
...
impl<T: fidl::endpoints::RequestStream> DeviceStream<T> {
    ...
    pub async fn recv_ready(
        &mut self,
    ) -> Result<(u32, fidl_fuchsia_guest_device::VirtioDeviceReadyResponder), DeviceStreamError>
    {
        match await!(self.recv_queue_or_ready()) {
...
impl<'a, T: 'a + Clone> DeviceBuilder<T> {
    pub async fn wait_for_ready<I: fidl::endpoints::RequestStream>(self, con: &'a mut DeviceStream<I>) -> Result<(Device<T>, GuestMem, fidl_fuchsia_guest_device::VirtioDeviceReadyResponder), Error> {
...
impl DeviceBuilder<NoEventMarker> {
    pub async fn wait_for_ready<I: fidl::endpoints::RequestStream>(self, con: &mut DeviceStream<I>) -> Result<(Device<NotifyEvent>, GuestMem, fidl_fuchsia_guest_device::VirtioDeviceReadyResponder), Error> {
