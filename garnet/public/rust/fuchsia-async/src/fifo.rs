...
/// foo
pub struct BufferedEntryStream<'a, F: 'a, R: FifoEntry> {
    fifo: &'a F,
    buffer: Vec<R>,
    index: usize,
    items: usize,
...
impl<'a, F: 'a, R: FifoEntry> Unpin for BufferedEntryStream<'a, F, R> {}

impl<'a, F: 'a, R: FifoEntry> Drop for BufferedEntryStream<'a, F, R> {
    fn drop(&mut self) {
        unsafe {
